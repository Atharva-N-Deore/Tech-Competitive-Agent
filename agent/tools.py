# LLM "tool use" (also called "function calling") works like this:
# 1. You define tools as JSON Schema objects and pass them in the API call.
# 2. The model reads the descriptions and decides if it needs to call a tool.
# 3. If yes, it responds with a tool_calls block instead of (or alongside) text.
# 4. YOUR code executes the actual Python function (in tool_executor.py).
# 5. You send the result back, and the model continues reasoning.
#
# We use the OpenAI tool format here because LiteLLM normalizes everything to it,
# regardless of which provider (Anthropic, Groq, Ollama, etc.) is configured.
#
# OpenAI format wraps each tool in {"type": "function", "function": {...}} and
# uses "parameters" for the schema. Anthropic's native format used "input_schema"
# directly — LiteLLM handles the translation internally when targeting Anthropic.
#
# The "description" field is critical: the model reads it to decide WHEN to call
# the tool. Write descriptions from the model's perspective: "use this when..."
    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "source_type": {
                    "type": "string",
                    # "enum" restricts valid values — the model cannot pass "Github" or
                    # "Jobs" (wrong case). Prevents hallucinating values not in our DB.
            ...
            # "required" lists fields the model MUST provide — it will not call this
            # tool without supplying these. limit is not required because it has a default.
...
# Having the model call save_analysis explicitly (rather than auto-saving) gives it
